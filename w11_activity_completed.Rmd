...
```{r}
# Completed
summary(lm(y ~ x, group_data))
```

Based on the linear model you fit above, which ONE of the following conclusions is appropriate?

**A) For each one-unit increase in `x` we expect `y` to decrease by 0.6 units, on average.** (correct)  
...
D) **As some of the observations in group 1 are very close to the line for group 2, we are likely to have concerns about the RMSE if we were to fit models to training and testing sets and compare.** (this one is false)
...
D) **No, in the model from question 2 there appears to be a negative linear association but in question 4' s model there appears to be positive linear association.** (correct)
E) Impossible to say.  
...
D) **The variable `z` appears to confound the relationship between the variables `x` and `y`.** (correct)  
...
B) **Limit the rate at which we scrape the page to 10 seconds per call.** (correct)
...
Below is some code that was previously run to get the data. THIS CODE IS NOT UP TO DATE, AND IS NOT BEING ASSESSED IN STA130. You aren't responsible for understanding it, but I thought some of you might be interested. 
...
write.csv(social_data, "scraped_data.csv")
```
...
A) Facebook
B) Instagram
C) **Twitter** (correct, see code below)
D) LinkedIn
E) YouTube
...
C) **This poses disclosure risk as other members of a tutorial group might be able to identify someone in their tutorial group based on gender and college.** (correct)
...
B) **Random assignment hopefully means the two groups are comparable across potential confounding variables, like high school preparation or convenience of time zone.** (correct)
...
Italics added above for greater clarity if you're using this question for revision. B was still obviously the best answer. 
...
A) Randomized control trial.
B) **Prospective cohort study.** (correct)
C) Retrospective cohort study.
D) Longitudinal study.
